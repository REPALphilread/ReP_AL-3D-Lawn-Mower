...
// Loads the EEPROM data into the GPS Fence
if (Fence == 10) {

 
  Serial.println(" ");
  Serial.println("Loading GPS Coordinates from EEPROM");
  Serial.println(" ");
  bool Show_Calculations = 0;               // make = 1 if you want to check the GPS Coordinates storage method is working in the Serial monitor
  GPSMAX = EEPROM.read(501);

  for (int i = 1; i <= (GPSMAX+1); i++) {
  ...
  // Thens eperate out the boundary condition of 1 or 2 from the EEPOM data slot.
  ...
  GPSY[i - 1] = (Part1 + Part2) * YNeg;               // The whole part of the number + the decimal part
  ...
  GPSX[i - 1] = (Part1 + Part2) * XNeg;               // The whole part of the number + the decimal part
  ...
  Boundary[i - 1] = Boundary_Calc;                             // BOundary Condition
...
if (WIFI_Enabled == true) Plot_GPS_Fence();
}
