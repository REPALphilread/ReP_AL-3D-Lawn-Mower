...
void Setup_Perimeter_Wire() {
  ...
  digitalWrite(Relay_Motors, HIGH);
  Serial.println(F("Main Power Off for Calibration"));
  delay(1500);
  lcd.clear();
  lcd.print("Calibrateing  ");
  lcd.setCursor(0, 1);
  ...
  delay(1500);
  Serial.println(F("Main Power Restored - Motors Ready"));
  }
...
void Power_Down_Mower() {
  Serial.println(F("Mower All Off"));
  Motion_StopMotors();
  delay(20);
  StopSpinBlades();
  delay(20);
  ...
  Serial.println(F("Main Power Off...."));
  Serial.println(F("Mower is in the Docking Station"));
  lcd.clear();
  lcd.print("Power Off");
