...
void Track_Wire_From_Dock_to_Zone_X() {
  ...
  for (I = 0; I < Track_Wire_Itterations; I++) {                                              // Iterations 'I' before mower leaves the wire.
    //  if (WIFI_On == 1) {
    //    if (  (I = 10) || (I = 1000) || (I =2000) || (I = 3000) || (I = 3900)   ) {
    //      //Get_Blynk_Commands();
    //    }
    //  }
      delay(5);
...
void FindWireTrack()  {
  ...
  for (int i = 0; i <= 1; i++) {
    ...
    if (inside == true)  {                                            // If the Mower is situated inside the wire then run the following code.
      ADCMan.run();
      UpdateWireSensor();
      Serial.println(F("CODE POSITION - MOTOR FORWARDS LOOP: inside If statements"));
      PrintBoundaryWireStatus();
      Motion_StopMotors();
      delay(1000);
  ...
  if ((Error == 7) && (Mower_Docked == 0)) {
     if (Track_Wire_CW_Charge == 1) {
        ...
        Serial.println(F("CW Tracking Selected"));
     }
     if (Track_Wire_CCW_Charge == 1) {
        SetPins_ToTurnLeft(); 
        Serial.println(F("CCW Tracking Selected")); 
     }
  }
  else {
    Serial.println(F("LEFT INPUT HERE............"));
    SetPins_ToTurnLeft();
    }
...
void Check_Wire_Blockage() {
    ...
    if (distance_blockage < maxdistancesonar) {
      ...
      Serial.println(F("**** Find Wire Blockage Registered ******"));
    }   
  ...
  if (Blockage_Registered >= Max_Number_Blockage_Registered) {
      Serial.println(F("Blockage Detected while finding wire...."));
      Motion_StopMotors();
      delay(2000);
      lcd.setCursor(0, 8);
